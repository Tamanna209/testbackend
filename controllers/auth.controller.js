...
//       return res.status(403).json({
//         message:
//           "You have already attempted this test. Multiple attempts are not allowed.",
//       });
//     }
...
const registerUser = async (req, res) => {
  try {
    // phone, college, technology intentionally ignored — dummy values used
    const { name, email } = req.body;
    ...
    // Phone validation commented out — dummy value "0000000000" sent from frontend
    // const phoneClean = (phone || "").toString().replace(/\D/g, "");
    // if (!/^\d{10}$/.test(phoneClean)) {
    ...
    if (user) {
      ...
      user = await User.findByIdAndUpdate(
        ...
        {
          name,
          // phone, college, technology intentionally NOT updated
          // hasAttemptedTest is intentionally NOT reset here
        },
        { new: true }
      );
    } else {
      // Phone uniqueness check removed — phone field no longer unique or required
      // const existingByPhone = await User.findOne({ phone: phoneClean });
      // if (existingByPhone) {
      //   return res.status(400).json({
      ...
      user = await User.create({
        name,
        email: emailLower,
        // phone, college, technology not passed — model defaults will apply:
        // phone: "0000000000", college: "N/A", technology: "N/A"
      });
    }

    console.log(`[DEV] Register request for email=${emailLower}`);
